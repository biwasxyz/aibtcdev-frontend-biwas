...
export const fetchWalletTokensByDao = async (daoId: string): Promise<WalletToken[]> => {
    const { data, error } = await supabase
        .from("holders")
        .select("*")
        .eq("dao_id", daoId)
...
export const fetchWalletTokensByWallet = async (walletId: string): Promise<WalletToken[]> => {
    const { data, error } = await supabase
        .from("holders")
        .select("*")
...
export const fetchWalletTokensByToken = async (tokenId: string): Promise<WalletToken[]> => {
    const { data, error } = await supabase
        .from("holders")
        .select("*")
        .eq("token_id", tokenId)
...
export const fetchWalletToken = async (tokenId: string): Promise<WalletToken | null> => {
    const { data, error } = await supabase
        .from("holders")
        .select("*")
        .eq("id", tokenId)
